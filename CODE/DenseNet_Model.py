...
def DenseNet(X_input, depth=16, nb_dense_block=2, growth_rate=16, nb_filter=32, dropout_rate=None, weight_decay=1E-4, up_conv = False):
# def DenseNet(img_dim, depth=16, nb_dense_block=2, growth_rate=16, nb_filter=32, dropout_rate=None, weight_decay=1E-4, up_conv = True):
    ...
    x = Conv2D(nb_filter, (3, 3),
               ...
               kernel_regularizer = l2(weight_decay))(X_input)
    ...
    # meu = Dense(nb_classes, activation='relu',kernel_regularizer=l2(weight_decay),bias_regularizer=l2(weight_decay))(x)
    # sigma = Dense(nb_classes, activation='relu',kernel_regularizer=l2(weight_decay),bias_regularizer=l2(weight_decay))(x)
    x = Conv2D(4, (3, 3), activation='relu',
               kernel_initializer="he_uniform",
               padding="same",
               use_bias=False,
               kernel_regularizer=l2(weight_decay))(x)
    ...
    if up_conv:
        x = Conv2DTranspose(4, (3, 3), activation='relu',
                   kernel_initializer="he_uniform",
                   padding="same",
                   use_bias=False,
                   kernel_regularizer=l2(weight_decay))(x)
...
# model = DenseNet(img_dim=(65, 65, 4), depth=7, nb_dense_block=17, growth_rate=4, nb_filter=8, dropout_rate=None, weight_decay=1E-4)
#
# # Model output
# model.summary()